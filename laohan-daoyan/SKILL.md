...
- `renderer_track: NATIVE` 时禁止在 EDL 写固定 `visual.render_plan` 代替 animation brief；⑪分别提交 native proposal。`renderer_track: PARITY` 只用于旧 render_plan 的等义执行/回归，不代表视觉上限。
...
在 09-导演/ 写入：导演简报.md、beat-sheet.md、edl.json、animation-brief.json、animation-ast.json、source-manifest.json、renderer-brief.md、styleframes/、animatic-manifest.json、review-checklist.md。`edl.json` 与 `source-manifest.json` 都必须记录 `edit_manifest_sha256`，并引用同一当前 edit-manifest。格式遵循项目 动画生产接口规格.md。
...
🔴 CHECKPOINT：REVIEW_GATED 的 EDL、animation brief、styleframe 与 animatic 必须获得审阅结论，才能交⑩或⑪；AST 重新编译后 SHA 必须与审阅输入一致。
