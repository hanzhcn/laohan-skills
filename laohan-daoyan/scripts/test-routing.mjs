...
if (!/^version:\s*["']?2\.1\.0["']?\s*$/m.test(skill) || !/^description:.*V5\.1导演预制.*WAITING_FOR_FOOTAGE/m.test(skill)) throw new Error('laohan-daoyan元数据必须把V5.1两段导演预制声明为默认入口');
for (const token of ['UNDERSTAND_CONTENT', 'DIVERGE_2_TO_3_DIRECTIONS', 'CONVERGE_ONE_COHERENT_PLAN', 'DENSITY_REVIEW', 'ADVANCED_EXPRESSION_CHALLENGE', 'COHERENT_RECONVERGENCE', 'director_draft: COMPLETED', 'director_review: PENDING', 'director_review: COMPLETED', 'method-v5.json', 'director-state-v5.md', 'LIGHTWEIGHT_RESULT_ONLY', '唯一新增业务产物', 'status: WAITING_FOR_FOOTAGE', '_status.md', 'check-episode-contract.sh', '不得引用窗口开始时的旧PASS']) {
