...
`risk_status` 是给 bianpai 的机械结论：只有高风险已清零且没有待处理高风险时写 `CLEAR` 与 `unresolved_high_risk_count: 0`。任何高风险、无法判断的高风险语境或需要回②改稿的情况都写 `REVISE_REQUIRED`（或无法完成扫描时写 `BLOCKED`），并写入实际未解决数。不得只靠 `script_hash` 让报告放行。
