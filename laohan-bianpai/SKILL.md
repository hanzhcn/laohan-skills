...
5. 输出唯一下一步的 skill、必要输入、应落盘产物与不能跨越的 gate。`AUTONOMOUS_RUN` 在标准①—⑥输出 `AUTO_CONTINUE_REQUIRED`；D1导演初稿与D2最终复审分别继续对应固定任务，D2完成后才路由⑦。标准入口到⑦输出 `WAITING_FOR_JEFFREY_SHOOTING` 并停下，这是计划内人工交接，不是 `BLOCKED`。只有当前 episode 记录 Jeffrey 本人、ISO时间和原话的 `DEFERRED_UNTIL_CANDIDATE_SELECTION` 时，⑥才标记为延后；具备完整`FULL_PIPELINE_TO_PUBLISH`授权时则使用`AUTO_DEFAULT_RANK_01_AFTER_CANDIDATE`，全程不得增加新的Jeffrey确认卡点。
...
- 新期②必须使用 `laohan-chuangzuo` schema 3，并实际通过其 `scripts/check-script-contract.mjs`：默认使用“嘿”开场，题目有明显更强的具体钩子时允许登记理由后改用题目专属开场；独立内容单位、至少一项真实原创增量、可视化锚点、逐段SHA、两遍内容重复审计、至少4种人味设备、自然稿长、连续分层编号和本机TTS缺一不可。旧episode按其 executor lock 中的1.6.x继续验证schema 2，不静默迁移。
