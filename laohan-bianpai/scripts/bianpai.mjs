...
const topicState = () => safely(() => {
  if (exists('00-选题-candidates.json')) {
    ...
    if ([3, 4].includes(candidatesPreview.schema_version)) {
      ...
      if (contract.status !== 0) {
        if (contractMessage.includes('TOPIC_RESCAN_REQUIRED')) return {done: false, reason: contractMessage};
        return {done: false, reason: `TOPIC_CONTRACT_BLOCKED：${contractMessage || '选题机械合同失败'}`};
      }
    }
  }
  ...
  if (config.schema_version === 2) {
    ...
    const validCreatorFit = (item) => {
      const fit = item.creator_fit;
      const bases = new Set(['FIRSTHAND_EXPERIENCE', 'BUILT_OR_TESTED', 'OWNED_EVIDENCE', 'LONG_TERM_PRACTICE', 'RESEARCHED_JUDGMENT', 'NOT_ESTABLISHED']);
      return fit && bases.has(fit.basis) && typeof fit.why_jeffrey === 'string' && fit.why_jeffrey.trim() && typeof fit.distinctive_judgment === 'string' && fit.distinctive_judgment.trim() && (item.disposition !== 'SELECTED' || fit.basis !== 'NOT_ESTABLISHED');
    };
    ...
    const validV2Candidate = (item) => {
      if (typeof item.event_cluster_id !== 'string' || !item.event_cluster_id.trim() || typeof item.why_now !== 'string' || !item.why_now.trim() || !['opinion-video', 'tutorial-video'].includes(item.content_form) || !['BEGINNER', 'INTERMEDIATE', 'ADVANCED'].includes(item.audience_level) || typeof item.audience_promise !== 'string' || !item.audience_promise.trim() || !validCreatorFit(item) || !nonEmptyStrings(item.assumed_prerequisites) || !nonEmptyStrings(item.jargon_to_explain) || typeof item.platform_query_intent !== 'string' || !item.platform_query_intent.trim() || !validAlignment(item) || !validContentGap(item) || !validClaimMap(item) || !validTutorialProof(item)) return false;
    ...
    if (topic.schema_version !== 4 && (![1, 2, 3].includes(candidates.schema_version) || !validTime(candidates.collected_at) || candidateItems.length < 2 || !uniqueNonEmpty(candidateIds) || !uniqueNonEmpty(candidateEvidenceIds) || !candidateEvidence.every(validEvidence) || (candidates.schema_version >= 2 && !validScreening) || !candidateItems.every((item) => validBaseCandidate(item) && (candidates.schema_version === 1 || validV2Candidate(item))) || candidateItems.filter((item) => item.disposition === 'SELECTED').length !== 1)) return {done: false, reason: 'candidates 必须留下 signals→longlist→短名单链路，引用真实 evidence、七维分数完整且只有一个 SELECTED；schema 2/3 同时锁定 Jeffrey 个人依据、独特判断、why-now、lane、小白合同、claim map 与平台语义状态'};
    ...
    if (topic.schema_version !== 4 && (topic.selected_candidate_id !== selected.id || !uniqueNonEmpty(topic.rejected_candidate_ids) || !topic.rejected_candidate_ids.every((id) => rejectedIds.includes(id)) || typeof topic.selection_rationale !== 'string' || !topic.selection_rationale.trim() || !topic.evidence.every((item) => selected.evidence_ids.includes(item.id)))) return {done: false, reason: '最终选题必须绑定唯一 SELECTED、非空淘汰项、选择理由及其候选 evidence'};
    if (candidates.schema_version >= 2 && candidates.schema_version <= 3 && (topic.schema_version !== candidates.schema_version || topic.content_form !== selected.content_form || topic.audience_level !== selected.audience_level || topic.audience_promise !== selected.audience_promise || JSON.stringify(topic.creator_fit) !== JSON.stringify(selected.creator_fit) || JSON.stringify(topic.assumed_prerequisites) !== JSON.stringify(selected.assumed_prerequisites) || JSON.stringify(topic.jargon_to_explain) !== JSON.stringify(selected.jargon_to_explain) || topic.platform_query_intent !== selected.platform_query_intent || JSON.stringify(topic.platform_alignment) !== JSON.stringify(selected.platform_alignment) || JSON.stringify(topic.content_gap) !== JSON.stringify(selected.content_gap) || JSON.stringify(topic.claim_evidence_map) !== JSON.stringify(selected.claim_evidence_map))) return {done: false, reason: 'schema 2/3 最终选题必须原样绑定 SELECTED 的 Jeffrey个人依据、lane、小白合同、平台对齐、内容缺口与 claim evidence map'};
  ...
  return {done: true, reason: topic.schema_version >= 4 ? '五步法选题已绑定数据层信号、假阳性过滤、单期/系列判定、冲突角度、收藏价值与AI自主定题理由' : topic.schema_version >= 2 ? '选题决策已绑定唯一内容 lane、小白合同、PRIMARY/平台语义对齐、Jeffrey门槛与可测实验' : '历史选题决策已绑定候选、PRIMARY/平台证据、抖音搜索与可测实验'};
}, '00-选题.json 无法读取');
const scriptState = () => safely(() => {
  ...
  if (!nonEmptyFile('01-口播稿.md') || !exists('02-创作工作稿/创作决策.json')) {
    const topicPreview = safely(() => readJson('00-选题.json'), null);
    const fiveStepContract = topicPreview?.schema_version === 4;
    if (fiveStepContract) return {done: false, reason: '②必须同时有非空 01-口播稿.md 与 02-创作工作稿/创作决策.json（五步法合同：从选题+表达池取材直接写稿，无采访/大纲确认门槛）'};
...
- 本窗口到candidate验收为止；封面、finalize与发布由后续独立Prompt路由。`);
  ...
  else if (next.id === '①' && topicState().reason.includes('TOPIC_RESCAN_REQUIRED')) console.log(`# 当前阶段：TOPIC_RESCAN_REQUIRED\n\nplanned_manual_gate: false\nblocked: false\n${fields}\n\n- ${topicState().reason}\n- 五步法信号不足或Jeffrey否决：按原因扩大扫描后重新定题。`);
  ...
  else if (next.id === '②' && scriptState().reason.includes('WAITING_FOR_JEFFREY_INTERVIEW')) console.log(`# 当前阶段：WAITING_FOR_JEFFREY_INTERVIEW（历史合同）\n\nplanned_manual_gate: true\nblocked: false\n${fields}\n\n- ${scriptState().reason}\n- 历史episode的采访合同；新期schema 4选题无此门槛。`);
  else if (next.id === '②' && scriptState().reason.includes('WAITING_FOR_JEFFREY_OUTLINE_APPROVAL')) console.log(`# 当前阶段：WAITING_FOR_JEFFREY_OUTLINE_APPROVAL（历史合同）\n\nplanned_manual_gate: true\nblocked: false\n${fields}\n\n- ${scriptState().reason}\n- 历史episode的大纲确认合同；新期schema 4选题无此门槛。`);
...
if (next) {
  ...
  else if (next.id === '①' && topicState().reason.includes('TOPIC_RESCAN_REQUIRED')) console.log('TOPIC_RESCAN_REQUIRED planned_manual_gate=false blocked=false');
  else if (next.id === '①' && topicState().reason.includes('TOPIC_CONTRACT_BLOCKED')) console.log('TOPIC_CONTRACT_BLOCKED blocked=true');
  else if (next.id === '②' && scriptState().reason.includes('WAITING_FOR_JEFFREY_INTERVIEW')) console.log('WAITING_FOR_JEFFREY_INTERVIEW planned_manual_gate=true blocked=false legacy=true');
  else if (next.id === '②' && scriptState().reason.includes('WAITING_FOR_JEFFREY_OUTLINE_APPROVAL')) console.log('WAITING_FOR_JEFFREY_OUTLINE_APPROVAL planned_manual_gate=true blocked=false legacy=true');
